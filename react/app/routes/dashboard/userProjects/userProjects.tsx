...
export default function UserProjects({ projects }: Props) {
  const [selectedProject, setSelectedProject] = useState<ProjectPresenter | null>(null);
    ...
    return (
      <Card variant="surface" className="p-8 text-center">
        ...
        <Text size="3" color="gray" mb="4">
          Get started by creating your first project. Projects help you organize your servers and deployments.
        </Text>
    ...
    <Box>
      ...
      <Grid columns={{ initial: "1", sm: "2", lg: "3" }} gap="4">
        {projects.map((project) => (
          <Card key={project.id} variant="surface" className="p-0 overflow-hidden hover:shadow-md transition-shadow">
            <Box className="p-4">
            ...
            <Box className="p-4">
              <Flex align="center" gap="2" mb="3">
                ...
                <Text size="2" weight="medium">
                  {project.servers?.length || 0} Server{project.servers?.length !== 1 ? 's' : ''}
                </Text>
              </Flex>
              ...
              {project.servers && project.servers.length > 0 && (
                <Box mb="4">
                  ...
                  <Flex direction="column" gap="1">
                    {project.servers.slice(0, 3).map((server: any, index: number) => (
                      <Flex key={index} align="center" gap="2" className="py-1">
                        <Box className={(server.active ? "bg-green-400" : "bg-red-400") + " w-1.5 h-1.5 rounded-full"} />
                        <Text size="2" className="flex-1 truncate">
                          {server.name || `Server ${index + 1}`}
                        </Text>
                        <Badge color={server.active ? "indigo" : "red"} variant="soft" size="1">
              ...
              <Flex gap="2">
                <Button size="2" variant="soft" className="flex-1" onClick={() => setSelectedProject(project)}>
                  View Details
                </Button>
                <Button size="2" variant="outline" className="flex-1">
                  Manage
                </Button>
              </Flex>
            </Box>
      ...
      <Card variant="surface" className="mt-6 p-4">
        <Heading size="4" mb="3">Quick Overview</Heading>
        <Grid columns={{ initial: "2", sm: "4" }} gap="4">
          ...
          <Box className="text-center">
            <Text size="6" weight="bold" className="block text-green-600">
              {projects.reduce((total, project) => total + (project.servers?.length || 0), 0)}
            </Text>
            <Text size="2" color="gray">Total Servers</Text>
          </Box>
          <Box className="text-center">
          ...
          <Box className="text-center">
            <Text size="6" weight="bold" className="block text-orange-600">
              {Math.round(projects.reduce((total, project) => total + (project.servers?.length || 0), 0) / projects.length * 10) / 10}
            </Text>
            <Text size="2" color="gray">Avg Servers/Project</Text>
          </Box>
        </Grid>
      </Card>
      ...
      <Card variant="surface" className="mt-4 p-4">
        <Flex justify="between" align="center" mb="3">
          ...
          <Button size="1" variant="ghost">View All</Button>
        </Flex>
          ...
          {projects.slice(0, 3).map((project, index) => (
            <Flex key={project.id} align="center" gap="3" className="py-2">
              ...
              <Box className="flex-1">
                ...
                <Text size="1" color="gray">
                  {index === 0 ? 'Server deployment completed' :
                    index === 1 ? 'Configuration updated' :
                      'Health check passed'}
                </Text>
              </Box>
              <Text size="1" color="gray">
